import { IMerjoonServiceBase, IMerjoonUsers, IMerjoonTasks, IMerjoonProjects } from '../common/types';
import { ShortcutApi } from './api';
import { ShortcutTransformer } from './transformer';
import { IShortcutWorkflowStateInfo } from './types';

export class ShortcutService implements IMerjoonServiceBase {
  ...
  private getWorkflowStateName(workflowId: number, stateId: number) {
    const state = this.workflowStates?.find(
      (state) => state.workflowId === workflowId && state.stateId === stateId,
    );
